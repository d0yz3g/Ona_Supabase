...
"""
        ...
        fallback_responses = [
            "Интересный вопрос! К сожалению, сейчас у меня возникли технические трудности с подключением к серверу. Не могли бы вы повторить свой вопрос чуть позже?",
            "Мне нужно немного времени, чтобы обдумать этот вопрос. Давайте вернемся к нему через несколько минут?",
            "Похоже, произошла небольшая техническая заминка. Я бы с радостью ответил на ваш вопрос, но нужно немного подождать, пока восстановится соединение с сервером.",
            "Я ценю ваш интерес к этой теме! К сожалению, сейчас у меня нет доступа к базе знаний из-за технических проблем. Не могли бы вы задать вопрос позже или сформулировать его иначе?",
            "Ваш запрос требует глубокого анализа, но сейчас у меня ограниченный доступ к ресурсам обработки. Можем ли мы вернуться к этому вопросу через несколько минут?"
        ]
...
async def get_personality_type_from_profile(profile_text: str) -> str:
    """
    Определяет тип личности на основе текста профиля.
    
    Args:
        profile_text: Текст профиля пользователя
        
    Returns:
        str: Тип личности ("Интеллектуальный", "Эмоциональный", "Практический" или "Творческий")
    """
    # Ключевые слова для каждого типа личности
    keywords = {
        "Интеллектуальный": ["логик", "аналитик", "рациональный", "критическое мышление", "интеллект", "знания", "анализ", "структура", "систематизация"],
        "Эмоциональный": ["эмпатия", "чувства", "эмоции", "забота", "поддержка", "сопереживание", "интуиция", "отношения", "гармония"],
        "Практический": ["результат", "действие", "эффективность", "конкретика", "практичность", "организованность", "план", "дисциплина"],
        "Творческий": ["креативность", "воображение", "творчество", "идеи", "вдохновение", "оригинальность", "нестандартный", "инновационный"]
    }
    
    # Приводим текст к нижнему регистру
    profile_text_lower = profile_text.lower()
    
    # Считаем количество совпадений для каждого типа
    matches = {}
    for personality_type, words in keywords.items():
        count = sum(1 for word in words if word in profile_text_lower)
        matches[personality_type] = count
    
    # Находим тип с наибольшим количеством совпадений
    max_matches = 0
    dominant_type = "Интеллектуальный"  # По умолчанию
    
    for personality_type, count in matches.items():
        if count > max_matches:
            max_matches = count
            dominant_type = personality_type
    
    logger.info(f"Определен тип личности: {dominant_type} (найдено {max_matches} совпадений)")
    return dominant_type

# Функция для регистрации обработчиков коммуникации
def register_communication_handlers(dp):
    """
    Регистрирует все обработчики коммуникации в диспетчере.
    
    Args:
        dp: Диспетчер
    """
    # Включаем роутер коммуникаций в диспетчер
    dp.include_router(communication_handler_router)
    
    # Логируем успешную регистрацию
    logger.info("Обработчики коммуникации успешно зарегистрированы") 