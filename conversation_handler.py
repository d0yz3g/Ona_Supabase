...
async def show_profile(message: Message, state: FSMContext):
    ...
    Args:
        ...
        state: Состояние FSM
    """
    # Импортируем функцию для показа профиля из survey_handler
    from survey_handler import command_profile
    
    # Вызываем функцию показа профиля
    await command_profile(message, state)

# Функция для регистрации обработчиков диалогов
def register_conversation_handlers(dp):
    """
    Регистрирует все обработчики диалогов в диспетчере.
    
    Args:
        dp: Диспетчер
    """
    # Включаем роутер диалогов в диспетчер
    dp.include_router(conversation_router)
    
    # Логируем успешную регистрацию
    logger.info("Обработчики диалогов успешно зарегистрированы") 