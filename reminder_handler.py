...
async def save_reminder_days(callback: CallbackQuery, state: FSMContext):
    ...
    days_text = ", ".join([day_names[day] for day in selected_days])
    
    await callback.message.edit_text(
        f"⏰ <b>Дни напоминаний изменены</b>\n\n"
        f"Вы будете получать напоминания о практиках в {time_str} в {days_text}.",
        reply_markup=get_reminder_keyboard(),
        parse_mode="HTML"
    )
    
    await callback.answer("Дни напоминаний сохранены!")
