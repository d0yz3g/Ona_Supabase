...
def generate_profile_prompt(answers: Dict[str, str]) -> str:
    ...
    # Формируем промт
    prompt = f"""Создай психологический профиль для пользователя с именем "{name}".

Информация о типе личности:
- Основной тип: {primary_type}
- Дополнительный тип: {secondary_type}
- Распределение ответов: A ({type_counts['A']}), B ({type_counts['B']}), C ({type_counts['C']}), D ({type_counts['D']})

Структура профиля должна включать:
1. Приветствие с именем пользователя
2. Сильные стороны (3-5 конкретных сильных сторон с учетом типа личности)
3. Личностные особенности (3-5 ключевых особенностей)
4. Зоны роста (2-3 области для развития, сформулированные позитивно)
5. Рекомендации (2-3 конкретные практические рекомендации)

Стиль и ограничения:
- Используй научный подход, без эзотерики и астрологии
- Пиши простым, понятным языком
- Акцентируй внимание на сильных сторонах
